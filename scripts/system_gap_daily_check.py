#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""system-gap-master daily gate — check/mark the once-per-day-per-host sync ritual.

Usage:
    python system_gap_daily_check.py check [--dir <SYNC_DIR>] [--host <NAME>]
    python system_gap_daily_check.py mark  [--dir <SYNC_DIR>] [--host <NAME>] [--note "..."]

Resolution order for the yard directory: --dir, then the
SYSTEM_GAP_MASTER_DIR environment variable, then the deprecated
SYNC_MASTER_DIR compatibility variable. The host defaults to this machine's hostname
...
HEADER = (
    ...
    "> Gate for the system-gap-master daily ritual (see SKILL.md). Appended by\n"
    "> scripts/system_gap_daily_check.py mark. Do not edit rows by hand unless fixing mistakes.\n\n"
    "| date | host | note |\n|---|---|---|\n"
)
...
def resolve_dir(cli_dir: str | None) -> Path | None:
    raw = (
        cli_dir
        or os.environ.get("SYSTEM_GAP_MASTER_DIR")
        or os.environ.get("SYNC_MASTER_DIR")
    )
    if not raw:
        return None
...
def cmd_check(sync_dir: Path, host: str) -> int:
    ...
    if not log.exists():
        print(f"[system-gap-master] no {LOG_NAME} in {sync_dir} — gate inactive.")
        return 0
    if row_exists(log, today(), host):
        print(f"[system-gap-master] {host}: today's sync is done.")
        return 0
    print(
        f"[system-gap-master] {host}: no sync yet today — consider running the "
...
def cmd_mark(sync_dir: Path, host: str, note: str) -> int:
    ...
    log = sync_dir / LOG_NAME
    if row_exists(log, today(), host):
        print(f"[system-gap-master] {host}: already marked for today.")
        return 0
    if not log.exists():
    ...
    print(f"[system-gap-master] {host}: marked {today()}.")
    return 0


def main(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="system-gap-master daily gate")
    ap.add_argument("command", choices=["check", "mark"])
    ap.add_argument(
        "--dir",
        dest="dir",
        default=None,
        help="yard directory (or set SYSTEM_GAP_MASTER_DIR)",
    )
    ...
    if sync_dir is None:
        print(
            "[system-gap-master] ERROR: no yard directory — pass --dir or set "
            "SYSTEM_GAP_MASTER_DIR.",
            file=sys.stderr,
        )
        return 2
    if not sync_dir.exists() and args.command == "check":
        print(f"[system-gap-master] yard {sync_dir} does not exist — gate inactive.")
        return 0
