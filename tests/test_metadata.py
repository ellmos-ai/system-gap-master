...
class MetadataParityTests(unittest.TestCase):
    ...
    def test_readme_target_personas_contract(self):
        en_readme = (self.root / "README.md").read_text(encoding="utf-8")
        de_readme = (self.root / "README_de.md").read_text(encoding="utf-8")
        marketing_log = (self.root / "MARKETING-LOG.txt").read_text(encoding="utf-8")

        for persona in ["[PERSONA-01]", "[PERSONA-02]", "[PERSONA-03]", "[PERSONA-04]"]:
            self.assertIn(persona, en_readme, f"{persona} missing in README.md")
            self.assertIn(persona, de_readme, f"{persona} missing in README_de.md")
            self.assertIn(persona, marketing_log, f"{persona} missing in MARKETING-LOG.txt")

    def test_readme_differentiation_matrix_contract(self):
        en_readme = (self.root / "README.md").read_text(encoding="utf-8")
        de_readme = (self.root / "README_de.md").read_text(encoding="utf-8")

        self.assertIn("Competitive Differentiation Matrix", en_readme)
        self.assertIn("Raw Cloud Storage", en_readme)
        self.assertIn("10-Dimensionen-Vergleichsmatrix", de_readme)
        self.assertIn("Reiner Cloud-Speicher", de_readme)

    def test_bgb_521_disclaimer_contract(self):
        de_readme = (self.root / "README_de.md").read_text(encoding="utf-8")
