...
class TestSyncDailyCheck(unittest.TestCase):
    ...
    def test_cmd_check_nonexistent_dir(self):
        ...
        ret = system_gap_daily_check.main(["check", "--dir", str(non_existent), "--host", "TESTHOST"])
        self.assertEqual(ret, 0)

    def test_cmd_mark_and_check_flow(self):
        # 1. Initially check returns 0 because LOG_NAME does not exist yet (gate inactive)
        ret_initial = system_gap_daily_check.cmd_check(self.yard_dir, "TESTHOST")
        self.assertEqual(ret_initial, 0)

        # 2. Mark today's sync
        ret_mark = system_gap_daily_check.cmd_mark(self.yard_dir, "TESTHOST", "Initial test sync")
        self.assertEqual(ret_mark, 0)

        # 3. Check again -> should return 0 (synced today)
        ret_check_after = system_gap_daily_check.cmd_check(self.yard_dir, "TESTHOST")
        self.assertEqual(ret_check_after, 0)

        # 4. Verify log file exists and has content
        log_file = self.yard_dir / system_gap_daily_check.LOG_NAME
    ...
    def test_cmd_check_due_when_log_exists_without_host(self):
        ...
        log_file = self.yard_dir / system_gap_daily_check.LOG_NAME
        log_file.write_text(system_gap_daily_check.HEADER + "| 2026-01-01 | OTHERHOST | note |\n", encoding="utf-8")

        # Now check for TESTHOST -> should return 1 (sync due)
        ret = system_gap_daily_check.cmd_check(self.yard_dir, "TESTHOST")
        self.assertEqual(ret, 1)

    def test_resolve_dir_env(self):
        os.environ["SYSTEM_GAP_MASTER_DIR"] = self.temp_dir
        try:
            resolved = system_gap_daily_check.resolve_dir(None)
            self.assertEqual(resolved, Path(self.temp_dir))
        finally:
            del os.environ["SYSTEM_GAP_MASTER_DIR"]

    def test_resolve_dir_legacy_env(self):
    ...
    def test_cli_missing_dir(self):
        env_backup = os.environ.pop("SYSTEM_GAP_MASTER_DIR", None)
        try:
            ret = system_gap_daily_check.main(["check"])
            self.assertEqual(ret, 2)
        finally:
            if env_backup:
                os.environ["SYSTEM_GAP_MASTER_DIR"] = env_backup
